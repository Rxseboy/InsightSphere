...
st.sidebar.subheader('Select the method of input:')
# Adding radio buttons for the user to choose between Uploading csv and getting stock data from the net 
option = st.sidebar.radio('Radio', ["Upload the data (.csv format)","Get data from the net"])
...
class stock_predict_DL:
    ...
    def LSTM_model(self):
        ...
        plt.xticks(range(0, len(self.y_test), 50), self.testd[::50], rotation=45)
